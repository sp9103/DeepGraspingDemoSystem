#include "ColorBasedTracker_pregrasp.h"




ColorBasedTracker_pregrasp::ColorBasedTracker_pregrasp(void)
{
}
...
void ColorBasedTracker_pregrasp::InsertBackGround(cv::Mat Color, cv::Mat Depth){
	imgWidth = Color.cols;
...
cv::Mat ColorBasedTracker_pregrasp::subBackground(cv::Mat srcColor, cv::Mat srcDepth){
...
cv::Mat ColorBasedTracker_pregrasp::calcImage(cv::Mat src, cv::Mat depth){
...
cv::Mat ColorBasedTracker_pregrasp::DetectColorMap(cv::Mat rgb, cv::Mat subMap){
	...
	cv::dilate(output, output, cv::Mat(), cv::Point(-1, -1), 8);
	//cv::erode(output, output, cv::Mat(), cv::Point(-1, -1), 3);

	return output;
}

cv::Mat ColorBasedTracker_pregrasp::DeleteSub(cv::Mat map, cv::Mat src){
...
bool sort_vector_pair_pregrasp(std::pair<int, int> i, std::pair<int, int> j) {	return (i.second > j.second);	}
void ColorBasedTracker_pregrasp::FindNMaxBlob(int N, std::vector<std::pair<std::vector<cv::Point2i>, cv::Rect>> *dst){
	...
	if (blobCount > 2)		std::sort(tempVec.begin(), tempVec.end(), sort_vector_pair_pregrasp);
...
cv::Mat ColorBasedTracker_pregrasp::drawBlobMap(cv::Mat src, std::vector<std::pair<std::vector<cv::Point2i>, cv::Rect>> blobInfo){
...
cv::Rect ColorBasedTracker_pregrasp::FindHandBlob(std::vector<std::pair<std::vector<cv::Point2i>, cv::Rect>> src){
	...
	if(nonBoundBlobIdx.size() == 0)
		return outRect;

	int minX = 99999, minY = 99999, maxX = -99999, maxY = -99999;
	for(int i = 0; i < nonBoundBlobIdx.size(); i++){
...
cv::Mat ColorBasedTracker_pregrasp::calcImage(cv::Mat src, cv::Mat depth, cv::Mat *dstColor, cv::Mat *dstDepth){
	if(src.channels() == 4)	cv::cvtColor(src, src, CV_BGRA2BGR);

	double duration;                                                         //변수 설정
	duration = static_cast<double>(cv::getTickCount());       //초기 시작 시간 설정
	cv::Mat output, outDepth;
	cv::Mat backSub = subBackground(src, depth);
	cv::Mat YellowMap = DetectColorMap(src, backSub);
	cv::Mat MapSub = DeleteSub(YellowMap, backSub);
	...
	if(HandBox.x <= 0 || HandBox.y <= 0 || src.cols <= x_prime || src.rows <= y_prime)	output.create(0, 0, CV_8UC1);
		...
		output.create(src.rows, src.cols, src.type());
		outDepth.create(src.rows, src.cols, CV_32FC1);
		for(int h = 0; h < src.rows; h++)
			for(int w = 0; w < src.cols; w++){
				if(HandBox.x <= w && w <= x_prime && HandBox.y <= h && h <= y_prime){
					output.at<cv::Vec3b>(h,w) = src.at<cv::Vec3b>(h,w);
					outDepth.at<float>(h,w) = depth.at<float>(h,w);
				}
				else{
					output.at<cv::Vec3b>(h,w) = ColorBackGround.at<cv::Vec3b>(h,w);
					outDepth.at<float>(h,w) = DepthBackGround.at<float>(h,w);
				}
			}
	}
	
	*dstColor = output.clone();
	*dstDepth = outDepth.clone();
