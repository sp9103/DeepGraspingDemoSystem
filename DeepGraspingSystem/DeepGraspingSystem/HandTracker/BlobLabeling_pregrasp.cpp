...
BlobLabeling_pregrasp::BlobLabeling_pregrasp(void)
{
...
void BlobLabeling_pregrasp::Labeling(int pixelCount, cv::Mat src){
	_group.clear();
	if(src.channels() != 1){
...
cv::Mat BlobLabeling_pregrasp::findChunk(cv::Mat src, std::map<int, std::vector<cv::Point2i>> *tGroup){
	int count = 0;
...
void BlobLabeling_pregrasp::visMap(cv::Mat src, const char *windowName){
...
void BlobLabeling_pregrasp::ChunkMerge(cv::Mat src, std::map<int, std::vector<cv::Point2i>> tGroup){
...
int BlobLabeling_pregrasp::findMinNeighborID(int h, int w, cv::Mat srcMap){
	float tempVal = -1;
...
int BlobLabeling_pregrasp::getBlobCount(){
	return _group.size();
}

std::vector<cv::Point2i> BlobLabeling_pregrasp::getBlob(int i){
